...
def main(args):
    ...
    PATH_TO_LABELS = os.path.join('dataset', 'class-labels.pbtxt')
    NUM_CLASSES = 2
    ...
    out = cv2.VideoWriter('out.mp4',fourcc, 15.0, (width, height))
    #out = cv2.VideoWriter('output.avi', -1, 30.0, (width,height))
    # Detection
    with detection_graph.as_default():
